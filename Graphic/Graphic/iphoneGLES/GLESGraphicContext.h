#if !defined ( _GLES_GRAPHIC_CONTEXT_H_ )
#define _GLES_GRAPHIC_CONTEXT_H_
...
namespace Graphic
{
    ...
    public:
        ...
        BOOL Initialize( VOID* pView );
        VOID Destroy();
        BOOL AdjustFrameBuffer( VOID* pView );

        inline VOID* GetContext() const { return m_pContext; }
        ...
        virtual SHADER_PROGRAM_HANDLE GetShaderProgram();
        virtual VOID ResetShaderProgram();
    ...
    private:
        void*                           m_pContext;
        ViewPort                        m_viewport;
        RENDER_TARGET_HANDLE            m_renderTarget;
        SURFACE_HANDLE                  m_depthSurface;
        SURFACE_HANDLE                  m_colorSurface;
        VERTEX_BUFFER_FORMAT_HANDLE     m_pVertexFormat;
        UINT32                          m_VertexAttributeFlag;
        SHADER_PROGRAM_HANDLE           m_currentShaderProgram;
        SHADER_PROGRAM_HANDLE           m_defaultShaderProgram;
        VERTEX_SHADER_HANDLE            m_defaultVertexShader;
        PIXEL_SHADER_HANDLE             m_defaultPixelShader;
    };
}
