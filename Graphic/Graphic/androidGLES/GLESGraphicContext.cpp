...
namespace Graphic
{
    ...
    BOOL GLESGraphicContext::Initialize( int w, int h )
    {
        // Create default framebuffer object.
        GLESFrameBuffer* pFrameBuffer = new GLESFrameBuffer( 0 );
        ...
        FakeGLESRenderSurface* renderSurface = new FakeGLESRenderSurface( w, h );
        ...
        renderSurface = new FakeGLESRenderSurface( w, h );
        if ( NULL == renderSurface )
        {
            Destroy();
            return FALSE;
    ...
    BOOL GLESGraphicContext::AdjustFrameBuffer(  int w, int h )
    {
        ...
        GLESRenderSurface* renderSurface = new GLESRenderSurface( bufferID, w, h );
        if ( NULL == renderSurface )
        {
        ...
        renderSurface = new GLESRenderSurface( bufferID, w, h);
        if ( NULL == renderSurface )
        {
            glDeleteRenderbuffers( 1, &bufferID );
            return FALSE;
    ...
    VOID GLESGraphicContext::SetVertexBuffer( VERTEX_BUFFER_HANDLE vtxHandle, VERTEX_BUFFER_FORMAT_HANDLE format )
    {
        ...
        if ( NULL != m_pVertexFormat )
        {
            ...
            for ( UINT32 i = 0; i < count; ++i )
            {
                ...
                glVertexAttribPointer(desc.index, desc.size, desc.type, GL_FALSE, pFormat->GetStride(), (GLvoid*)desc.offset);
